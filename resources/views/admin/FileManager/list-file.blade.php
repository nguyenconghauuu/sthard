                    ...
                    <table class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th style="width:65%">Nội dung khóa học</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($course_main_contents->first())
                                    @foreach($course_main_contents as $course_main_content)
                                        @if($course_main_content->course_id == $course->id)
                                            <ul>
                                                <li><b>{!! $course_main_content->chapter_content !!}</b>
                                                    @if($course_file_contents->first())
                                                        @foreach($course_file_contents as $course_file_content)
                                                            @if($course_file_content->course_id == $course->id && $course_main_content->chapter_content == $course_file_content->chapter)
                                                                <ul class="ul-file">
                                                                    <li><span>{!! $course_file_content->file_content !!}</span>
                                                                        <div class="table-group-icon">
                                                                            <div class="group-icon">
                                                                                <a href="{{route('admin.show-file-content-course' , $course_file_content->id)}}"><i style="font-size:14px" class="fa-solid fa-pen-to-square"></i></a>
                                                                            </div>
                                                                            <div class="group-icon">
                                                                                <a href="{{route('admin.destroy-file-content-course' , $course_file_content->id)}}"><i class="fa-solid fa-trash-can"></i></a>
                                                                            </div>
                                                                            <div class="group-icon">
                                                                                <a href="{{route('user.course-content' , $course_file_content->course_id)}}"><i class="fa-solid fa-display"></i></a>
                                                                            </div>
                                                                        </div>
                                                                    </li>
                                                                </ul>
